...
1. **Install Alacritty**:
   ...
   - On macOS:
   Zsh is pre-installed. You can check by running:
  ```bash
  zsh --version
  ```
  - On Debian/Ubuntu:
  ...
  sudo apt install zsh
  ```
    - After Alacritty installed.
2. **Install zsh(if not installed)**:
   Zsh is a Unix shell. Just like Bourne shell but with more improvement.
   We need Zsh to use *Oh My Zsh*.
   > You can download it from the official [Alacritty Website](https://alacritty.org/), make sure to get the latest version. Or you can also install it using your system's package manager.
